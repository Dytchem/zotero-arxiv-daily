...
- **Full-pool visibility** — the agent sees every deduplicated paper from today's fetch, including ones the keyword/min-score/max-count filters dropped. A high-value paper that the heuristics missed can still be rescued, read and recommended — it shows up in the email among the "Other candidates" (rescued papers are listed by relevance like any candidate).
...
- **Provider-safe by design** — the LLM provider is created programmatically from the base URL hardcoded in config (`llm.api.base_url`, default `https://opencode.ai/zen/go/v1`) + the `LLM_API_KEY` secret with *only* your configured model; Pi's built-in provider catalog (which can silently fall back to unconfigured models like xiaomi/mimo) is loaded and immediately cleared, leaving exactly one custom provider.
