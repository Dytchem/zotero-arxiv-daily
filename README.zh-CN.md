...
4. **让自主 agent 做决定** —— 它浏览当天*全部*论文（不只是初筛名单），自己抓全文精读（长文派子 agent 分块阅读）、上网核实出处、给每篇论文打**推荐度**（0–10）。
5. **发一封精美的 HTML 邮件**：导语、专家排序的卡片（带 **Relevance** 相关度徽章和 **推荐度** 徽章），以及完整的"其他候选"区 —— 每个候选都打了分，不静默丢弃任何一篇。
...
- **成本可控** —— agent 先从摘要给全池打分，只对短名单（约 8 篇）精读；长文子 agent 每请求分块 16k 字符。单次运行费用远低于 $0.20。无硬限制 —— agent 觉得需要时可以读更多。
