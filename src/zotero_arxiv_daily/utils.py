    ...
    for i, q_tokens in enumerate(_tokenize(q) for q in query_texts):
        ...
        for q in q_tokens:
            ...
            for j, tf in enumerate(tf_counts):
                f_q = tf.get(q, 0)
                scores[i, j] += idf * (f_q * (k1 + 1)) / (f_q + k1 * (1 - b + b * dl[j] / max(avgdl, 1)))
    return scores
...
def _bm25_pick(query: str, candidates: dict[str, str], k1: float = 1.5, b: float = 0.75) -> str:
    ...
    for name, tokens in doc_tokens.items():
        ...
        if score > best_score:
            best_score = score
            best_name = name
    # Zero lexical overlap with every candidate: fall back to the first one
    # instead of returning None (symmetric with the empty-query branch).
    return best_name if best_name is not None else next(iter(candidates))
